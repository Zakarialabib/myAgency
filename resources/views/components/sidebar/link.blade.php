...
@php
    $isActiveClasses =  $isActive ? 'bg-blue-900 text-white hover:bg-blue-700' : 'text-zinc500 hover:text-zinc700 hover:bg-zinc100 dark:hover:text-zinc300 dark:hover:bg-dark-eval-2';  
...
@if ($collapsible)
    <button type="button" {{ $attributes->merge(['class' => $classes]) }} >
        ...
        <span  x-show="isSidebarOpen || isSidebarHovered" aria-hidden="true" class="relative block ml-auto w-6 h-6">
            <span :class="open ? '-rotate-45' : 'rotate-45'" class="absolute right-[9px] bg-zinc400 mt-[-5px] h-2 w-[2px] top-1/2 transition-all duration-200"></span>
            <span :class="open ? 'rotate-45' : '-rotate-45'" class="absolute left-[9px] bg-zinc400 mt-[-5px] h-2 w-[2px] top-1/2 transition-all duration-200"></span>
        </span>
    </button>
@else
