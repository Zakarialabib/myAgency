<div class="relative inline-block w-60 mr-2 align-middle select-none transition duration-200 ease-in">
    <select name="{{$name}}" id="{{$id}}" @if($checked) checked @endif {{ $attributes->merge(['class'=>"form-select p-3 leading-5 bg-white dark:bg-dark-eval-2 text-zinc700 dark:text-zinc300 rounded border border-zinc300 mb-1 text-sm w-full focus:shadow-outline-blue focus:border-blue-500"])}}/>
        ...
        <label for="{{$id}}" class="block overflow-hidden h-6 rounded-full bg-zinc300 cursor-pointer"></label>
    </select>
</div>