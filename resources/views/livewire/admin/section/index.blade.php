            ...
            @forelse($sections as $section)
                <x-table.tr class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                    <x-table.td id="accordion-collapse" data-accordion="collapse">
                        <div id="accordion-collapse-heading-{{ $section->id }}">
                            <button type="button"
                    ...
                    <x-table.td>
                        ...
                        @elseif($section->page == \App\Models\Section::TEAM_PAGE)
                            <a href=""
                                class="bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-900">
                                {{ __('Team') }}
                            </a>
                        ...
                        @elseif($section->page == \App\Models\Section::PORTFOLIO_PAGE)
                ...
                <tr id="accordion-collapse-body-{{ $section->id }}" class="hidden"
                    aria-labelledby="accordion-collapse-heading-{{ $section->id }}">
                    <td colspan="12">
                        <div class="panel-body text-center p-5 flex flex-wrap">
                            <div class="w-1/2">

                                <h1>{{ __('Title') }} : {{ $section->title }}</h1>
                                <p>{{ __('Subtitle') }} : {{ $section->subtitle }}</p>
                                <p>{{ __('Content') }} : {!! $section->content !!}</p>
                                <p>{{ __('Text') }} : {{ $section->text }}</p>
                            </div>
                            <div class="w-1/2">
                                <p class="py-4">{{ __('Custom button') }} : </p>
                                <a class="bg-green-500 py-4 my-4 px-6" href="{{ $section->link }}">
                                    {{ $section->button }}
                                </a>
