<div>
    ...
    <x-card>
        ...
        <x-table>
            ...
            <x-table.tbody>
                @forelse($emails as $email)
                    <x-table.tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $email->id }}">
                        <x-table.td>
                            <input type="checkbox" value="{{ $email->id }}" wire:model="selected">
                        </x-table.td>
                        <x-table.td>
                            {{ $email->type }}
                        </x-table.td>
        ...
        @livewire('admin.email.edit', ['email' => $email])

        @livewire('admin.email.create')
